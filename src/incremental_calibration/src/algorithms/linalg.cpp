...
cholmod_sparse *columnSubmatrix(cholmod_sparse *A, std::ptrdiff_t colStartIdx,
                                std::ptrdiff_t colEndIdx,
                                cholmod_common *cholmod) {
  if (A == NULL)
    throw NullPointerException("A", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (colEndIdx < colStartIdx)
    throw OutOfBoundException<std::ptrdiff_t>(
        colEndIdx, colStartIdx, "colStartIdx must be lower than colEndIdx",
        __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (colEndIdx >= static_cast<std::ptrdiff_t>(A->ncol))
    throw OutOfBoundException<std::ptrdiff_t>(
        colEndIdx, A->ncol, "index must be lower than the number of columns",
        __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (colStartIdx < 0)
    throw OutOfBoundException<std::ptrdiff_t>(
        colStartIdx, 0, "columnSubmatrix(): index must be positive", __FILE__,
  ...
  cholmod_sparse *A_sub =
      cholmod_l_submatrix(A, NULL, -1, colIndices, numIndices, 1, 1, cholmod);
  delete[] colIndices;
  if (A_sub == NULL)
...
cholmod_sparse *rowSubmatrix(cholmod_sparse *A, std::ptrdiff_t rowStartIdx,
                             std::ptrdiff_t rowEndIdx,
                             cholmod_common *cholmod) {
  if (A == NULL)
    throw NullPointerException("A", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (rowEndIdx < rowStartIdx)
    throw OutOfBoundException<std::ptrdiff_t>(
        rowEndIdx, rowStartIdx, "rowStartIdx must be lower than rowEndIdx",
        __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (rowEndIdx >= static_cast<std::ptrdiff_t>(A->nrow))
    throw OutOfBoundException<std::ptrdiff_t>(
        rowEndIdx, A->nrow, "index must be lower than the number of rows",
        __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (rowStartIdx < 0)
    throw OutOfBoundException<std::ptrdiff_t>(
        rowStartIdx, 0, "index must be positive", __FILE__, __LINE__,
        __PRETTY_FUNCTION__);
  if (cholmod == NULL)
    throw NullPointerException("cholmod", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  ...
  cholmod_sparse *A_sub =
      cholmod_l_submatrix(A, rowIndices, numIndices, NULL, -1, 1, 1, cholmod);
  delete[] rowIndices;
  if (A_sub == NULL)
    throw InvalidOperationException("cholmod_l_submatrix failed", __FILE__,
                                    __LINE__, __PRETTY_FUNCTION__);
...
double colNorm(const cholmod_sparse *A, std::ptrdiff_t j) {
  ...
  if (j >= static_cast<std::ptrdiff_t>(A->ncol))
    throw OutOfBoundException<std::ptrdiff_t>(
        j, A->ncol, "index must be lower than the number of columns", __FILE__,
        __LINE__, __PRETTY_FUNCTION__);
  if (j < 0)
    throw OutOfBoundException<std::ptrdiff_t>(j, 0, "index must be positive",
                                              __FILE__, __LINE__,
                                              __PRETTY_FUNCTION__);
...
cholmod_dense *columnScalingMatrix(cholmod_sparse *A, cholmod_common *cholmod,
  ...
  if (G == NULL)
    throw InvalidOperationException("cholmod_l_allocate_dense failed", __FILE__,
                                    __LINE__, __PRETTY_FUNCTION__);
...
void cholmodSparseToEigenDenseCopy(const cholmod_sparse *in,
                                   Eigen::MatrixXd &out) {
  if (in == NULL)
    throw NullPointerException("in", __FILE__, __LINE__, __PRETTY_FUNCTION__);
...
void cholmodDenseToEigenDenseCopy(const cholmod_dense *in,
                                  Eigen::VectorXd &out) {
  if (in == NULL)
    throw NullPointerException("in", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  out.resize(in->nrow);
...
cholmod_sparse *eigenDenseToCholmodSparseCopy(const Eigen::MatrixXd &in,
  ...
  cholmod_sparse *out = cholmod_l_allocate_sparse(
      in.rows(), in.cols(), nzmax, 1, 1, 0, CHOLMOD_REAL, cholmod);
  if (out == NULL)
...
double qrTol(cholmod_sparse *A, cholmod_common *cholmod, double eps) {
  ...
  return 20.0 * static_cast<double>(A->nrow + A->ncol) * eps *
         cholmod_l_norm_sparse(A, 1, cholmod);
}
...
void reduceLeftHandSide(SuiteSparseQR_factorization<double> *factor,
                        cholmod_sparse *A_rt, cholmod_sparse **Omega,
  ...
  if (factor == NULL || factor->QRsym == NULL || factor->QRnum == NULL)
    throw NullPointerException("factor", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  if (A_rt == NULL)
    throw NullPointerException("A_rt", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (cholmod == NULL)
    throw NullPointerException("cholmod", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  if (Omega == NULL)
  ...
  if (A_rtQ2 == NULL)
    throw InvalidOperationException("cholmod_l_aat failed", __FILE__, __LINE__,
                                    __PRETTY_FUNCTION__);
  A_rtQ2->stype = 1;
  ...
  *Omega = cholmod_l_add(A_rt2, A_rtQ2, alpha, beta, 1, 1, cholmod);
  ...
  cholmod_l_free_sparse(&A_rtQ2, cholmod);
  if (*Omega == NULL)
    throw InvalidOperationException("cholmod_l_add failed", __FILE__, __LINE__,
                                    __PRETTY_FUNCTION__);
}

cholmod_dense *reduceRightHandSide(SuiteSparseQR_factorization<double> *factor,
                                   cholmod_sparse *A_rt, cholmod_sparse *A_rtQ,
                                   cholmod_dense *b, cholmod_common *cholmod) {
  if (factor == NULL || factor->QRsym == NULL || factor->QRnum == NULL)
    throw NullPointerException("factor", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  if (A_rt == NULL)
    throw NullPointerException("A_rt", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (b == NULL)
    throw NullPointerException("b", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (A_rtQ == NULL)
    throw NullPointerException("A_rtQ", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  if (cholmod == NULL)
  ...
  cholmod_sparse *QtbFull =
      SuiteSparseQR_qmult<double>(SPQR_QTX, factor, bSparse, cholmod);
  cholmod_l_free_sparse(&bSparse, cholmod);
  if (QtbFull == NULL) {
...
void analyzeSVD(const cholmod_sparse *Omega, Eigen::VectorXd &sv,
  ...
  const Eigen::JacobiSVD<Eigen::MatrixXd> svd(
      OmegaDense, Eigen::ComputeThinU | Eigen::ComputeThinV);
  U = svd.matrixU();
  V = svd.matrixV();
...
cholmod_dense *solveQR(SuiteSparseQR_factorization<double> *factor,
                       cholmod_dense *b, cholmod_sparse *A_r,
  ...
  if (b == NULL)
    throw NullPointerException("b", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (A_r == NULL)
    throw NullPointerException("A_r", __FILE__, __LINE__, __PRETTY_FUNCTION__);
  if (x_r.size() != static_cast<int>(A_r->ncol))
    throw OutOfBoundException<int>(x_r.size(), A_r->ncol,
                                   "mismatch between x_r and A_r", __FILE__,
                                   __LINE__, __PRETTY_FUNCTION__);
  if (cholmod == NULL)
    throw NullPointerException("cholmod", __FILE__, __LINE__,
                               __PRETTY_FUNCTION__);
  ...
  cholmod_dense *A_rx_r =
      cholmod_l_allocate_dense(A_r->nrow, 1, A_r->nrow, CHOLMOD_REAL, cholmod);
  if (A_rx_r == NULL)
    throw InvalidOperationException("cholmod_l_allocate_dense failed", __FILE__,
                                    __LINE__, __PRETTY_FUNCTION__);
  double alpha[2];
  ...
  cholmod_dense *QtbmA_rx_r =
      SuiteSparseQR_qmult<double>(SPQR_QTX, factor, &bmA_rx_r, cholmod);
  if (QtbmA_rx_r == NULL)
    throw InvalidOperationException("SuiteSparseQR_qmult failed", __FILE__,
                                    __LINE__, __PRETTY_FUNCTION__);
  cholmod_dense *x_l = SuiteSparseQR_solve<double>(SPQR_RETX_EQUALS_B, factor,
                                                   QtbmA_rx_r, cholmod);
